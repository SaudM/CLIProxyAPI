...
func TestClaudeExecutor_NonClaudeRequestUsesClaudeCode220CLIFingerprint(t *testing.T) {
	...
	assertClaudeFingerprint(t, seenHeaders, "claude-cli/2.1.274 (external, cli)", "0.112.1", "v26.3.0", "MacOS", "arm64")
	...
	if got := system[0].Get("text").String(); got != "x-anthropic-billing-header: cc_version=2.1.274.834; cc_entrypoint=cli;" {
		t.Fatalf("billing header = %q, want 2.1.274 CLI fingerprint", got)
	}
...
func TestClaudeExecutor_ConfirmedVSCodeAgentSDKRequestPreservesIdentity(t *testing.T) {
	...
	const vscodeUA = "claude-cli/2.1.274 (external, claude-vscode, agent-sdk/0.3.220)"
	const billingHeader = "x-anthropic-billing-header: cc_version=2.1.274.9cb; cc_entrypoint=claude-vscode;"
...
func TestClaudeExecutor_CopiedVSCodeAgentSDKHeadersWithoutMetadataAreCloaked(t *testing.T) {
	...
	_, errExecute := executor.Execute(context.Background(), auth, cliproxyexecutor.Request{
		...
		OriginalRequest: payload,
		Headers: http.Header{
			"User-Agent":     {"claude-cli/2.1.274 (external, claude-vscode, agent-sdk/0.3.220)"},
			"X-App":          {"cli"},
			"Anthropic-Beta": {"claude-code-20250219"},
		},
	})
	...
	if got := seenHeaders.Get("User-Agent"); got != "claude-cli/2.1.274 (external, cli)" {
...
func TestClaudeExecutor_AgentSDKEntrypointWithStrongSignalsUsesCLICloak(t *testing.T) {
	...
	_, errExecute := executor.Execute(context.Background(), auth, cliproxyexecutor.Request{
		...
		OriginalRequest: payload,
		Headers: http.Header{
			"User-Agent":     {"claude-cli/2.1.274 (external, sdk-ts, agent-sdk/0.3.220)"},
			"X-App":          {"cli"},
			"Anthropic-Beta": {"claude-code-20250219"},
		},
	})
...
func TestClaudeBillingFingerprintUsesFirstUserText(t *testing.T) {
	...
	// Two native captures pin the build-hash algorithm across releases: the
	// 2.1.258 official capture and a 2.1.274 capture taken 2026-09-18 with
	// CLAUDE_CODE_ATTRIBUTION_HEADER enabled (cc_version=2.1.274.913).
	...
	if got := computeFingerprint("Please summarize the repository layout in one sentence.", "2.1.274"); got != "913" {
		t.Fatalf("computeFingerprint() = %q, want native 2.1.274 capture suffix 913", got)
	}
}
...
func TestCheckSystemInstructionsWithSigningMode_LongPromptIsExactAndIdempotent(t *testing.T) {
	...
	first := checkSystemInstructionsWithSigningMode(payload, false, true, "2.1.274", "cli", "")
	second := checkSystemInstructionsWithSigningMode(first, false, true, "2.1.274", "cli", "")
	if !bytes.Equal(first, second) {
...
func TestClaudeExecutor_UncloakedProbePreservesCallerBillingTags(t *testing.T) {
	...
	callerBilling := "x-anthropic-billing-header: cc_version=2.1.274; cc_entrypoint=cli; cch=abc12; cc_prompt_id=00000000-0000-4000-8000-000000000001;"
...
func TestClaudeExecutor_ExecuteOAuthCustomToolMCPAliasRoundTrip(t *testing.T) {
	...
	if got := upstreamHeaders.Get("User-Agent"); got != "claude-cli/2.1.274 (external, cli)" {
...
func TestClaudeCodeCLIBetas_MatchesObservedClientMatrix(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// Captured 2026-09-02 from Claude Code 2.1.274 (cli entrypoint, OAuth,
			// auto mode on): 158 inline tools without tool search, advisor beta
			// enabled for the account, thinking adaptive without display.
			name:  "2.1.274 main thread capture with inline tools and afk-mode",
		...
		},
		{
			...
			body:  `{"model":"claude-sonnet-5","system":[{"type":"text","text":"x-anthropic-billing-header: cc_version=2.1.274.0ab; cc_is_subagent=true;"}]}`,
			oauth: true,
			want: "claude-code-20250219,oauth-2025-04-20," +
				"interleaved-thinking-2025-05-14,redact-thinking-2026-02-12," +
				"thinking-token-count-2026-05-13,context-management-2025-06-27," +
...
// insertion point against every beta that follows it on the 2.1.274 wire,
...
func TestClaudeExecutor_CloakModePrefersStoredPrevReqOverCallerFake(t *testing.T) {
	...
	fakeCallerPayload := []byte(`{
		"model": "claude-sonnet-5",
		"system": [
			{"type": "text", "text": "x-anthropic-billing-header: cc_version=2.1.274.000; cc_entrypoint=cli; cch=00000; cc_prev_req=req_fake_caller_999; cc_prompt_id=aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee;"},
