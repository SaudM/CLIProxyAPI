...
func TestDetectClaudeCodeRequestRejectsEachMissingMessageSignal(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "x-app", headers: http.Header{"User-Agent": {"claude-cli/2.1.274 (external, cli)"}, "Anthropic-Beta": {"claude-code-20250219"}}, body: payload},
		{name: "user-agent", headers: http.Header{"User-Agent": {"curl/8.7.1"}, "X-App": {"cli"}, "Anthropic-Beta": {"claude-code-20250219"}}, body: payload},
		{name: "betas", headers: http.Header{"User-Agent": {"claude-cli/2.1.274 (external, cli)"}, "X-App": {"cli"}}, body: payload},
...
func TestDetectClaudeCodeRequestClassifiesEntrypoints(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "cli", userAgent: "claude-cli/2.1.274 (external, cli)", entrypoint: "cli", subclient: "claude-code-cli", native: true},
		{name: "vscode-agent-sdk", userAgent: "claude-cli/2.1.274 (external, claude-vscode, agent-sdk/0.3.220)", entrypoint: "claude-vscode", subclient: "claude-code-vscode", agentSDKVersion: "0.3.220", native: true},
		{name: "sdk-cli", userAgent: "claude-cli/2.1.274 (external, sdk-cli)", entrypoint: "sdk-cli", subclient: "claude-code-cli-sdk", native: true},
		{name: "sdk-ts", userAgent: "claude-cli/2.1.274 (external, sdk-ts, agent-sdk/0.3.220)", entrypoint: "sdk-ts", subclient: "claude-code-sdk-ts", agentSDKVersion: "0.3.220"},
		{name: "sdk-py", userAgent: "claude-cli/2.1.274 (external, sdk-py, agent-sdk/0.1.0)", entrypoint: "sdk-py", subclient: "claude-code-sdk-py", agentSDKVersion: "0.1.0"},
		{name: "desktop", userAgent: "claude-cli/2.1.274 (external, claude-desktop)", entrypoint: "claude-desktop", subclient: "claude-desktop"},
		{name: "desktop-third-party-inference", userAgent: "claude-cli/2.1.274 (external, claude-desktop-3p)", entrypoint: "claude-desktop-3p", subclient: "claude-desktop-3p"},
		{name: "remote", userAgent: "claude-cli/2.1.274 (external, remote)", entrypoint: "remote", subclient: "claude-remote"},
		{name: "github-action", userAgent: "claude-cli/2.1.274 (external, claude-code-github-action)", entrypoint: "claude-code-github-action", subclient: "claude-code-gh-action"},
		{name: "unknown", userAgent: "claude-cli/2.1.274 (external, copied-client)", entrypoint: "copied-client"},
	} {
		t.Run(test.name, func(t *testing.T) {
			headers := confirmedClaudeCodeHeaders()
...
func TestDetectClaudeCodeCountTokensAllowsMissingMetadata(t *testing.T) {
	headers := confirmedClaudeCodeHeaders()
	headers.Set("User-Agent", "claude-cli/2.1.274 (external, claude-vscode, agent-sdk/0.3.220)")
...
func TestDetectClaudeCodeRequestRejectsMalformedNativeSignals(t *testing.T) {
	tests := []struct {
		...
		{name: "unrelated beta", headers: http.Header{"User-Agent": {"claude-cli/2.1.274 (external, cli)"}, "X-App": {"cli"}, "Anthropic-Beta": {"anything"}}, userID: validClaudeCodeMetadataUserID},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
