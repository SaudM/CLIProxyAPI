...
func TestClaudeClientVersionsTracksLast24Hours(t *testing.T) {
	...
	RecordClaudeClientVersion("auth-a", "claude-cli/2.1.258 (external, cli)")
	RecordClaudeClientVersion("auth-a", "claude-cli/2.1.258 (external, cli)")
	RecordClaudeClientVersion("auth-a", "claude-cli/2.1.301 (external, sdk-cli)")
	RecordClaudeClientVersion("auth-a", "curl/8.0")
	RecordClaudeClientVersion("", "claude-cli/2.1.258 (external, cli)")

	versions := ClaudeClientVersions("auth-a")
	if len(versions) != 2 || versions[0].Version != "2.1.258" || versions[0].Requests != 2 || versions[1].Version != "2.1.301" || versions[1].Requests != 1 {
	...
	if version, ok := ClaudeClientVersionFromUserAgent("claude-cli/2.1.258 (external, cli)"); !ok || version != "2.1.258" {
